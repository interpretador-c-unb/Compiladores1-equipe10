...
1. **Gerar o arquivo do Bison**  
   ```bash
   bison -d interpretador_c.y
   ```
   - Isso criará dois arquivos: `interpretador_c.tab.c` e `interpretador_c.tab.h` (o `-d` gera o `.h` que contém as definições dos tokens).

2. **Gerar o arquivo do Flex**  
   ```bash
   flex interpretador_c.l
   ```
   - Isso criará um arquivo `lex.yy.c`.

3. **Compilar tudo junto**  
   Use um compilador C (por interpretador_c, *gcc*) para gerar o executável:
   ```bash
   gcc -o parser interpretador_c.tab.c lex.yy.c -lfl
   ```
